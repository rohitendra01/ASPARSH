...
exports.listPortfolios = async (req, res) => {
  try {
    const portfolios = await portfolioRepository.findPortfoliosByTenant(null);
...
exports.showCreateForm = async (req, res) => {
  try {
    const profile = req.params?.slug ? await require('../repositories/profileRepository').findProfileDocumentBySlugAndTenant(req.params.slug, null) : null;
...
exports.updatePortfolio = async (req, res) => {
  try {
    const portfolio = await portfolioRepository.updatePortfolio(req.params.id, null, req.body, req.user._id);
    if (!portfolio) return res.status(404).json({ success: false, message: 'Portfolio not found' });
...
exports.deletePortfolio = async (req, res) => {
  try {
    await portfolioService.processPortfolioDeletion(req.params.id, null);
...
exports.listUserPortfolios = async (req, res) => {
  try {
    const portfolios = await portfolioRepository.findPortfoliosByTenant(null, req.params.profileId);
